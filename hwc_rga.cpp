...
inline int HEIGHT(const hwc_rect &rect)
{
...
hwc_rect_t calculate_src_crop_center(hwc_rect_t src_rect, hwc_rect_t dst_rect, int transform)
{
    ...
    HWC_UNREFERENCED_PARAMETER(src_rect);
    HWC_UNREFERENCED_PARAMETER(dst_rect);
    HWC_UNREFERENCED_PARAMETER(transform);
...
void rga_set_fds_offsets(struct rga_req *rga_request, uint16_t src_fd, uint16_t dst_fd, uint32_t src_offset, uint32_t dst_offset)
{
...
int set_rga_cfg(hwc_cfg_t  *cfg)
{
    ...
    if (cfg->rga_fbFd == 0)
    {
        g_rga_cfg.rga_cfg.src.yrgb_addr = 0;
        g_rga_cfg.rga_cfg.src.uv_addr = SRC_HANDLE_BASE;
    }
    else
    {
        //g_rga_cfg.rga_cfg.src.yrgb_addr = cfg->rga_fbAddr+0x60000000;
        g_rga_cfg.rga_cfg.src.yrgb_addr = cfg->rga_fbFd;
    }
    ...
    g_rga_cfg.rga_cfg.dst.yrgb_addr = 0;
    g_rga_cfg.rga_cfg.dst.uv_addr = DST_HANDLE_BASE;
    ...
    g_rga_cfg.rga_cfg.mmu_info.mmu_flag  |= (1 << 31) | (1 << 10);
